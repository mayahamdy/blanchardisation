...
namespace Quantum.Task {
  ...
  public abstract unsafe class SystemThreadedFilter<T> : SystemBase where T : unmanaged {
    ...
    /// <inheritdoc cref="SystemArrayComponent{T}.OnInit(Frame)"/>
    public sealed override void OnInit(Frame frame) {
      frame.Context.TaskContext.RegisterDelegate(TaskThreadedFilter, GetType().Name + ".Update", ref _threadedTaskDelegateHandle);
      OnInitUser(frame);
    }
  ...
  public static partial class DebugCommand {
    ...
    private class InternalSystem : SystemMainThread {
      public override void Update(Frame frame) {
        for (int p = 0; p < frame.MaxPlayerCount; ++p) {
          if (frame.GetPlayerCommand(p) is InternalCommand cmd) {
            for (int i = 0; i < cmd.Data.Length; ++i) {
              Execute(frame, ref cmd.Data[i]);
            }
          }
        }
      }
  ...
  public unsafe class PlayerConnectedSystem : SystemMainThread {
    ...
    public override void Update(Frame frame) {
      ...
      for (int p = 0; p < frame.MaxPlayerCount; p++) {
        ...
        var playerLastConnectionStateRef = frame.PlayerLastConnectionState;
        
        if (isPlayerConnected != playerLastConnectionStateRef.IsSet(p)) {
          if (isPlayerConnected) {
            frame.PlayerConnectedCount++;
            frame.Signals.OnPlayerConnected(p);
          } else {
            frame.PlayerConnectedCount--;
            frame.Signals.OnPlayerDisconnected(p);
          }
